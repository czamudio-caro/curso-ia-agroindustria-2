...
\newpage
% =====================================================
% Capítulo 0 (Intro): Puente desde Python básico a HPC/NumPy
% =====================================================
...
\subsection{Control de flujo: decisiones y reglas}
...
\subsection{Funciones: empaquetar lógica reutilizable}
...
\subsection{Por qué NumPy: cuando el tamaño importa}
...
\subsection{Vectorización con ufuncs: ``sin for''}
...
Como ya estamos usando NumPy (\texttt{np}), el siguiente paso natural es graficar con Matplotlib a través de \texttt{matplotlib.pyplot}. [web:358]
...
El flujo típico con \texttt{pyplot} es: crear gráfico, etiquetar, y guardar/mostrar. [web:358]
...
plt.plot(hum, label="Humedad (%)")    # curva [web:354]
plt.title("Humedad por hora")
plt.xlabel("Hora")
plt.ylabel("Humedad (%)")
plt.legend()                          # leyenda [web:352]
plt.show()
\end{lstlisting}
...
np.random.seed(7)
temp = 20 + 10*np.random.rand(100)
hum  = 30 + 40*np.random.rand(100)

plt.scatter(temp, hum, s=18)
...
Para una matriz (por ejemplo, mapa de humedad), \texttt{imshow} la dibuja como imagen y \texttt{colorbar} añade la escala de colores. [web:387][web:382]
...
plt.imshow(humedad, cmap="viridis", interpolation="nearest")  # [web:387]
plt.colorbar()                                                # [web:382]
plt.title("Mapa de humedad (50x50)")
plt.show()
\end{lstlisting}

\subsection{Guardar gráficos con \texttt{savefig}}
Para incluir resultados en el reporte, lo ideal es guardar figuras en archivos (por ejemplo PNG) usando \texttt{savefig}. [web:351]
...
plt.savefig("mapa_humedad.png", dpi=150, bbox_inches="tight")  # [web:351]
\end{lstlisting}

\begin{warningbox}{Tip práctico}
Si vas a usar \texttt{plt.show()}, guarda antes con \texttt{plt.savefig(...)} para evitar perder la figura en algunos flujos de ejecución. [web:386]
\end{warningbox}
...
En Semana 01 construiste hábitos: terminal, organización y Git.
En Semana 02 vamos a profesionalizar el código: validar datos, medir rendimiento y usar NumPy para operar por bloques.
...
% =====================================================
\section{Capítulo I: Programación defensiva (Fail Fast)}
...
% =====================================================
\section{Capítulo II: CSV sucio \texorpdfstring{\(\rightarrow\)}{->} CSV limpio (sin pandas)}
...
% =====================================================
\section{Capítulo III: HPC con NumPy — benchmark (loop vs vectorización)}
...
% =====================================================
\section{Capítulo V: Reporte en LaTeX (manual en Semana 2)}

\begin{conceptbox}{Qué debe incluir tu PDF (1 página)}
\begin{itemize}
\item Resumen (5--8 líneas).
\item Resultados del benchmark (tiempos + speedup).
\item Resultados del lote (porcentajes + agua total).
\item Qué validaciones aplicaste y por qué.
\end{itemize}
\end{conceptbox}

\begin{agrobox}{Ejercicio 5 (entrega)}
En \texttt{workspace/report/main.tex} completa tu reporte y compila:
